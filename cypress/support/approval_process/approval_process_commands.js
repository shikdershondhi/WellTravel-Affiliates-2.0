...
Cypress.Commands.add('approval_process_pre_process',()=>{
	cy.get('.stepper').then(($body) => {
		if ($body.text().includes('Request Approval')) {
			...
			cy.get('.request__heading-approve-button > .button').click().wait(100)
			cy.travelapp_logout()
			cy.travelapp_login().wait(100)
			cy.get('.top-nav__avatar > .rcl-avatar > .rcl-avatar__content').click().wait(100)
			...
			cy.get('.request-list__rtabs > .rtabs > .rtabs__nav > .rtabs__nav-container > :nth-child(2)').click().wait(2000)
			cy.get(':nth-child(1) > .table__field--align-center > .table__field-value > .link > .link__content > .icon-button').click().wait(100)
			cy.get('.request__heading-status > :nth-child(2) > .button').scrollIntoView()
...
Cypress.Commands.add('approval_process_post_and_pdf_download',()=>{
	cy.get('.account-body > :nth-child(1)').then(($body) => {
		if ($body.text().includes('Start Approval Process')) {
			...
			cy.get(':nth-child(3) > .link').click().wait(100)
